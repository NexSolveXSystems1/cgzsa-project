...
async function targetFor(actorRank: number, userId: string) {
  const [row] = await db
    .select({ id: users.id, name: users.name, roleId: users.roleId, rank: roles.rank })
    .from(users)
...
export async function inviteUser(formData: FormData) {
  ...
  const [role] = await db
    .select({ id: roles.id, rank: roles.rank, label: roles.label })
    .from(roles)
    .where(eq(roles.id, roleId))
    .limit(1);
  if (!role) return { error: "Choose a role." };
  if (role.rank >= actor.rank) {
    return { error: "You cannot invite somebody at or above your own level." };
  }
...
export async function changeRole(userId: string, roleId: string) {
  ...
  const [after] = await db
    .select({ label: roles.label, rank: roles.rank })
    .from(roles)
    .where(eq(roles.id, roleId))
    .limit(1);
  if (!after) return { error: "That role does not exist." };
  if (after.rank >= actor.rank) {
    return { error: "You cannot grant a role at or above your own level." };
  }
