...
export async function savePage(id: string | null, formData: FormData) {
  ...
  const { title, section, excerpt, seoTitle, seoDescription, seoCanonical, seoNoindex } = parsed.data;
  const body = sanitiseHtml(parsed.data.body);

  const wanted = slugify(parsed.data.slug || title);
  const slug = await uniqueSlug(wanted, async (s) => {
    const [row] = await db
      .select({ id: pages.id })
      .from(pages)
  ...
  if (id) {
    ...
    await db.update(pages).set({ title, slug, section: section || null, excerpt: excerpt || null, body, updatedAt: new Date() }).where(eq(pages.id, id));
  ...
  const [created] = await db
    .insert(pages)
    .values({ title, slug, section: section || null, excerpt: excerpt || htmlToText(body).slice(0, 200), body, authorId: actor.id, status: "DRAFT" })
