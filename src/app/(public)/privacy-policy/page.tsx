...
export default async function Privacy() {
  ...
  const [page] = await db.select().from(pages).where(eq(pages.slug, "privacy-policy")).limit(1);

  if (page && page.status === "PUBLISHED" && !page.deletedAt) {
    return (
      <>
        <PageHead crumbs={[["Home", "/"], ["Privacy Policy"]]} eyebrow="Legal" title={page.title} lede={page.excerpt ?? undefined} />
