...
import {
  ...
  requestContext, requirePermission, tooManyAttempts, verifyPassword,
} from "@/lib/auth";
...
export async function claimConversation(conversationId: string) {
  const actor = await requirePermission("chat.answer").catch((e) => { throw e; });
  await db
...
export async function replyToConversation(conversationId: string, formData: FormData) {
  const actor = await requirePermission("chat.answer");
...
export async function closeConversation(conversationId: string) {
  const actor = await requirePermission("chat.close");
  await db
    .update(conversations)
