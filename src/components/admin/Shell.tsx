      ...
      <aside className="bg-[#0C1610] text-[#A9BEB2] p-3 lg:sticky lg:top-0 lg:h-screen lg:overflow-auto">
        <div className="flex items-center gap-2.5 px-2 pb-4 border-b border-white/10 mb-3">
          {logoStorageKey ? (
            ...
            <img src={`/api/media/${logoStorageKey}`} alt={s.orgName} className="h-[34px] w-auto max-w-[100px] object-contain shrink-0 rounded" />
          ) : (
            <span className="grid place-items-center w-8.5 h-8.5 rounded-full bg-white text-[var(--color-brand)] font-serif font-bold text-[0.75rem] shrink-0 w-[34px] h-[34px]">CG</span>
          )}
