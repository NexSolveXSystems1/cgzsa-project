...
/**
 * CSRF boundary for mutating server actions.
 *
 * SameSite=Lax already blocks most cross-site form posts from carrying the
 * session cookie. This origin check is the second line: if a browser or proxy
 * still sends a credentialed unsafe request, it must have come from this site.
 */
export async function requireSameOrigin() {
  const h = await headers();
  const rawOrigin = h.get("origin");

  if (!rawOrigin) {
    if (process.env.NODE_ENV === "production") throw new AuthError("Missing request origin");
    return;
  }

  const origin = asOrigin(rawOrigin);
  if (!origin) throw new AuthError("Invalid request origin");

  if (process.env.NODE_ENV !== "production") {
    const originHost = new URL(origin).hostname;
    if (originHost === "localhost" || originHost === "127.0.0.1" || originHost === "::1") return;
  }

  const host = firstForwarded(h.get("x-forwarded-host")) ?? h.get("host");
  const proto = firstForwarded(h.get("x-forwarded-proto")) ?? (process.env.NODE_ENV === "production" ? "https" : "http");
  const allowed = new Set<string>();

  if (host) allowed.add(`${proto}://${host}`);
  const appOrigin = asOrigin(process.env.APP_URL);
  if (appOrigin) allowed.add(appOrigin);

  if (!allowed.has(origin)) throw new AuthError("Untrusted request origin");
}

export async function requireActionActor(): Promise<Actor> {
  await requireSameOrigin();
  return requireActor();
}

export async function requireActionPermission(key: string): Promise<Actor> {
  await requireSameOrigin();
...
export async function requestContext() {
  ...
  return {
    ...
    // value the client can write. See cgzsa-backend/src/lib/request.ts.
