...
--
-- The guard below refuses the migration if any of them turns out to hold data â€”
...
BEGIN
  FOREACH t IN ARRAY ARRAY['subscribers','project_partners','project_documents','media_albums'] LOOP
    IF to_regclass('public.' || t) IS NOT NULL THEN
      ...
      IF n > 0 THEN
        RAISE EXCEPTION
          'Refusing to drop "%" â€” it holds % row(s). These tables were unused when the audit ran; something has written to this one since. Review the data before removing this guard.', t, n;
      END IF;
    END IF;
  END LOOP;
...
-- second always fails, which rolls the whole migration back â€” the drops appear
