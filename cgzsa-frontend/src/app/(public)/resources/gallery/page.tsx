...
export async function generateMetadata() {
  return cmsMetadata("resources/gallery", {
    path: "/resources/gallery",
    title: "Media Gallery",
    description: "Photographs from CGZSA clean-ups, installations and community work.",
    image: STORY_IMAGES.communityCleanup,
  });
}

export default async function Gallery() {
  const [images, cmsPage] = await Promise.all([
    db.select().from(mediaAssets)
      .where(isNull(mediaAssets.deletedAt)).orderBy(desc(mediaAssets.createdAt)).limit(120),
    getPublishedCmsPage("resources/gallery"),
  ]);
  ...
  return (
    <>
      <PageHead
        crumbs={[["Home", "/"], ["Resources"], ["Gallery"]]}
        eyebrow={cmsPage?.section ?? "Resources"}
        title={cmsPage?.title ?? "Media Gallery"}
        lede={cmsPage?.excerpt ?? "Photographs from our clean-ups, installations and community work."}
        image={image}
      />
