...
export async function generateMetadata() {
  return cmsMetadata("resources/publications", {
    path: "/resources/publications",
    title: "Publications & Documents",
    description: "Reports, policies and organizational documents, free to read and download.",
    image: STORY_IMAGES.publicSpace,
  });
}

export default async function Publications() {
  const [rows, cmsPage] = await Promise.all([
    db
      .select({
    ...
    getPublishedCmsPage("resources/publications"),
  ]);
  ...
  return (
    <>
      <PageHead
        crumbs={[["Home", "/"], ["Resources"], ["Publications"]]}
        eyebrow={cmsPage?.section ?? "Resources"}
        title={cmsPage?.title ?? "Publications & Documents"}
        lede={cmsPage?.excerpt ?? "Reports, policies and organizational documents, free to read and download."}
        image={image}
      />
