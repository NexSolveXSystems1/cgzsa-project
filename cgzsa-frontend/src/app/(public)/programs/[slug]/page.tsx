...
import { mediaStoryImage, programmeImage } from "@/components/public/storyImages";
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [p] = await db
    ...
    .where(and(eq(programs.slug, slug), eq(programs.status, "PUBLISHED"), isNull(programs.deletedAt)))
    .limit(1);
  if (!p) return {};
  const image = mediaStoryImage(p.imageKey, p.imageAlt, programmeImage(p.slug));
  return pageMetadata({
...
export default async function ProgramPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [p] = await db
    .select({
      ...
      rationale: programs.rationale,
      response: programs.response,
      icon: programs.icon,
      imageKey: mediaAssets.storageKey,
      imageAlt: mediaAssets.altText,
    })
  ...
  const image = p.imageKey
    ? { src: `/api/media/${p.imageKey}`, alt: p.imageAlt ?? p.title, credit: "CMS upload" }
    : programmeImage(p.slug);
  ...
  return (
    <>
      <PageHead crumbs={[["Home", "/"], ["Programmes", "/programs"], [p.title]]} eyebrow={p.tagline} title={p.title} lede={p.lead} image={image} />
      <section className="py-14">
        <div className="wrap grid gap-13 lg:grid-cols-[1fr_320px] items-start">
          <div className="prose-cg">
            <figure className="not-prose m-0 mb-8 overflow-hidden rounded-lg border border-[var(--color-line)] bg-white">
              <div className="story-image aspect-[21/10]">
            ...
            <div className="not-prose grid gap-3 sm:grid-cols-2 mb-8">
              {p.activities.map((a) => (
                <div key={a} className="border-l-[3px] border-[var(--color-brand)] bg-white px-4 py-3 text-[0.9rem] text-[var(--color-ink-2)]">
                  {a}
                </div>
              ))}
            </div>
          ...
          <aside className="grid gap-4 lg:sticky lg:top-28">
            <div className="card p-5">
              <div className="flex items-center gap-2.5 mb-3">
                <span className="grid place-items-center w-10 h-10 rounded-lg bg-[var(--color-brand-soft)] text-[var(--color-brand)] shrink-0">
                  <ProgramIcon name={p.icon} />
                </span>
                <p className="eyebrow m-0">Get involved</p>
              </div>
