...
import { db } from "@/db";
import { impactMetrics, mediaAssets, programs, projects } from "@/db/schema";
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [p] = await db
    ...
    .where(and(eq(projects.slug, slug), eq(projects.status, "PUBLISHED"), isNull(projects.deletedAt)))
    .limit(1);
  if (!p) return {};
  ...
  return (
    <>
      <PageHead crumbs={[["Home", "/"], ["Projects", "/projects"], [p.title]]}
        eyebrow={p.programme ?? "Project"} title={p.title} image={image} />
