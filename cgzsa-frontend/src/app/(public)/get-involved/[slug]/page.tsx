...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const fallback = slug === "volunteer" ? STORY_IMAGES.communityCleanup : slug === "partner" ? STORY_IMAGES.busStop : STORY_IMAGES.safeWater;
  const title = slug === "volunteer" ? "Volunteer With Us" : slug === "partner" ? "Partner With Us" : "Support Our Work";
  return cmsMetadata(`get-involved/${slug}`, {
    path: `/get-involved/${slug}`,
    title,
    description: "Join, partner with, or support CGZSA's environmental work.",
    image: fallback,
  });
}
...
export default async function GetInvolved({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const page = await getPublishedCmsPage(`get-involved/${slug}`);
  ...
  const fallback = slug === "volunteer" ? STORY_IMAGES.communityCleanup : slug === "partner" ? STORY_IMAGES.busStop : STORY_IMAGES.safeWater;
  const headImage = cmsImage(page, fallback);
  const fallbackTitle = slug === "volunteer" ? "Volunteer With Us" : slug === "partner" ? "Partner With Us" : "Support Our Work";
  const fallbackLede = slug === "volunteer"
    ? "Join clean-up days, awareness teams, media support or monitoring work."
    : slug === "partner"
      ? "Back a water point, park recovery, bus-stop bench or school campaign."
      : "Help fund tools, materials, installations and the training that keeps them working.";

  return (
    <>
      <PageHead
        crumbs={[["Home", "/"], ["Get Involved"], [page?.title ?? fallbackTitle]]}
        eyebrow="Get involved"
        title={page?.title ?? fallbackTitle}
        lede={page?.excerpt ?? fallbackLede}
        image={headImage}
      />
      <section className="py-14"><div className="wrap">
        ...
        <div className="grid gap-11 lg:grid-cols-2 items-start">
          <div className="prose-cg">
            {page?.body && <div dangerouslySetInnerHTML={{ __html: cmsBodyHtml(page.body) }} />}
