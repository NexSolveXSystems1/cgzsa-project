...
export async function generateMetadata() {
  return cmsMetadata("accessibility", {
    path: "/accessibility",
    title: "Accessibility Statement",
    description: "Our commitment, what we have done, and how to tell us when we fall short.",
    image: STORY_IMAGES.publicSpace,
  });
}

export default async function Accessibility() {
  const page = await getPublishedCmsPage("accessibility");
  if (page) return <CmsPage page={page} crumbs={[["Home", "/"], ["Accessibility"]]} eyebrow="Legal" fallbackImage={STORY_IMAGES.publicSpace} narrow />;
