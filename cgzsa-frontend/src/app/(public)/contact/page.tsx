...
export async function generateMetadata() {
  return cmsMetadata("contact", {
    path: "/contact",
    title: "Contact Us",
    description: "Contact CGZSA by message, email, telephone, or through the principal office.",
    image: STORY_IMAGES.busStop,
  });
}
...
export default async function Contact() {
  ...
  const [headerFallback, cmsPage] = await Promise.all([
    settingImage(s.contactPageImageId, STORY_IMAGES.busStop),
  ...
  return (
    <>
      <PageHead
        crumbs={[["Home", "/"], ["Contact"]]}
        eyebrow={cmsPage?.section ?? "Contact"}
        title={cmsPage?.title ?? "Contact Us"}
        lede={cmsPage?.excerpt ?? `Our office is at ${s.office.split(",").slice(0, 2).join(",")}. We answer every message.`}
        image={headerImage}
      />
