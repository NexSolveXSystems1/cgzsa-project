...
import { db } from "@/db";
import { mediaAssets, pageRevisions, pages, users } from "@/db/schema";
      ...
      <SaveForm action={savePage.bind(null, page?.id ?? null)}>
        <div className="grid gap-4.5 gap-5 lg:grid-cols-[minmax(0,1fr)_318px] items-start">
          ...
          <div className="grid gap-4">
            ...
            <Panel title="Page Image">
              <p className="hint mb-3">
                Used on this page header and as the default sharing image unless an SEO override is set.
              </p>
              <ImagePicker
                name="imageId"
                images={images.map((i) => ({ id: i.id, alt: i.altText, key: i.storageKey }))}
                selected={page?.imageId ?? null}
              />
            </Panel>
